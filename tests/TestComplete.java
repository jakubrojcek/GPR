...
public class TestComplete {
  ...
  that means 4 categories of traders. Fast have zero PV *//*
        ...
        *//* for (int j = 0; j < 13; j ++){
        ...
        Trader trader = new Trader(tauB, tauS, nP, tickSize, ReturnFrequencyHFT,
                ReturnFrequencyNonHFT, LL, HL, end, breakPoint);
        ...
        ID = book.randomNonHFTtraderID();
        tr = traders.get(ID);
        tr.decision(book.getRank(ID), book.getBookSizes(), book.getBookInfo(),
                EventTime, FV);
        long Bt = book.getBestBid();
        long At = book.getBestAsk();
        long lBt = book.getBookInfo()[2];    // depth at best Bid
        long lAt = book.getBookInfo()[3];    // depth at best Ask
        long dBt  = book.getBookInfo()[4];   // depth at buy
        int dSt = book.getBookInfo()[5];     // depth at sell
        int Pt = book.getBookInfo()[6];      // last transaction pricePosition position
        int b = book.getBookInfo()[7];       // 1 if last transaction buy, 0 if sell
        int P = book.getRank(ID).get(nP);    // pricePosition position in previous action
        int q = book.getRank(ID).get(P);     // priority of the order at P
        int x;                               // order was buy(2) or sell(1) or no order (0)
        int a = traders.get(ID).getPv();     // private value zero(0), negative (1), positive (2)
        int l = 0;                           // arrival frequency slow (0), fast (1)
        x = (book.getBookSizes()[P] > 0) ? 2 : 1;

        Long code = (Bt<<50) + (At<<44) + (lBt<<40) + (lAt<<36) + (dBt<<29) + (dSt<<22)
                + (Pt<<16) + (b<<15) + (P<<9) + (q<<5) + (x<<3) + (a<<1) + l;
        if (code == traders.get(ID).getOldCode() &&
                (code>>50) == Bt){
        ...
        //trader.printOccurrences(false, "occurrences.csv");
        System.out.println("Traders count in Trader " + Trader.TraderCount
                +  " trade count " + Trader.tradeCount + " from history " + h.historySize());
